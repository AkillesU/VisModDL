...
def main():
    ...
    model_info = {
        "source": config.get("model_source", "cornet"),
        "repo": config.get("model_repo", "-"),
        "name": config.get("model_name", "cornet_z"),
    ...
    eccentricity_bands = config.get("eccentricity_bands", [])
    ecc_fraction_to_mask_params = config.get("ecc_fraction_to_mask", [0, 21, 0.05])

    # Shared setup for the damage runs.
    common_kwargs = dict(
    ...
    run_damage(
        **common_kwargs,
        ...
        ecc_fraction_to_mask_params=config.get(
            "ecc_fraction_to_mask_gradual",
